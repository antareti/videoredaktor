...
export default function App() {
  ...
  const handleProcessImage = async () => {
    ...
    try {
      ...
      const result: GeminiResponse | null = await editImageWithGemini(base64Image, mimeType, prompt);
      ...
      setError(`Произошла ошибка: ${errorMessage}. Проверьте консоль для получения дополнительной информации.`);
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
        ...
        const videoUrl = await generateVideoFromImage(base64Image, mimeType, prompt);
